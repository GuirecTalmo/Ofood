...
export default function Dashboard({ getUserMeals, userMeals, currentUser }) {
  return (
    <main className="main_dashboard">
      <div className="head__container">
        ...
        <button
          className={
            currentUser?.profil_is_completed
              ? "button_generate"
              : "button_generate button_generate--forbidden"
          }
          type="button"
          onClick={getUserMeals}
          disabled={currentUser?.profil_is_complete}
        >
          Générer mon planing
        </button>
      </div>
      ...
      {currentUser?.profil_is_completed === false && (
        <div className="messageBx">
          <p className="dashboardMessage">
            Configurez votre profil afin d'avoir accès au menu !
          </p>
        </div>
      )}
              ...
              <div>
                ...
                <div className="card__container">
                  {day.recipesofuser.map((recipe) => (
                    <div
                      className="Card animated animatedFadeInUp fadeInUp"
                      key={recipe.id}
                    >
                      <Link to={`/recipes/${recipe.id}`} state={{ recipe }}>
                        <div className="container__picture">
                          <img
                            className="picture__card"
                            src={
                              recipe?.photo_link.charAt(0) === "h"
                                ? recipe.photo_link
                                : "https://img.cuisineaz.com/660x660/2016/06/27/i93803-porridge-aux-flocons-d-avoine.webp"
                            }
                            alt="avatar"
                          />
                        </div>
